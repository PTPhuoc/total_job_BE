...
def get_safe_value(soup, selector, attr=None, strip=True):
    ...
    if attr:
        return element.get(attr) if element else "Không tìm thấy!"
    else:
        return element.get_text(strip=strip) if element else "Không tìm thấy!"


def get_safe_passage(soup, selector):
    element = soup.select_one(selector)
    return "\n".join(element.stripped_strings) if element else "Không tìm thấy!"
...
def get_links(driver, get_more=1):
    list_link = []
...
def get_company_infor(driver, soup):
    field = get_safe_value(soup,
                           "div.job-detail__company--information > div.job-detail__company--information-item.company-field > div.company-value")
...
def get_detail_links(driver, links):
    list_job_detail = []
    for lk in links:
        try:
            print(f"Bắt đầu lấy dữ liệu từ: {lk}")
            driver.get(lk)
            soup_detail_page = wait_and_check(driver, ".job-detail__body", 5)
            if soup_detail_page:
                job = get_job_infor(soup_detail_page, lk)
                print("Lấy thông tin tuyển dụng thành công!")
                job_desc = get_job_desc(soup_detail_page)
                print("Lấy mô tả tuyển dụng thành công!")
                job_req = get_job_req(soup_detail_page)
                print("Lấy thông tin yêu cầu tuyển dụng thành công!")
                ...
                print("Không tìm thấy element chứa thông tin!")
        except Exception as ex:
            print(f"Không thể lấy dữ liệu được từ link: {lk}. Lỗi: {str(ex)}")
    print("\n-----Đã lấy xong, bắt đầu xuất-----\n")
...
def start_crawl():
    ...
    try:
        ...
        if len(links) > 0:
            detail_link = get_detail_links(driver, links[:5])
            if detail_link:
